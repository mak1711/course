...
def generate_launch_description():
    ...
    def run_yolo(context: LaunchContext, use_tracking, use_3d):
        ...
        # Skip `uv sync`'s isolated venv (it re-downloads a full ~2.5GB torch/cuda
        # stack on first run) -- this machine's system Python already has a working
        # torch+CUDA install and a close-enough ultralytics version (8.4.x) installed
        # via `pip install --user`, so just use PYTHONPATH as-is.
