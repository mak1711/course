...
std::shared_ptr<std::string> robot_description =
...
while (rclcpp::ok() && robot_description->empty())
{
      ...
      std::chrono::seconds(5))
  {
    RCLCPP_ERROR(
      get_node()->get_logger(),
      "Failed to receive robot_description topic"
    );
    return controller_interface::CallbackReturn::ERROR;
  }
...
urdf_string_ = *robot_description;
...
if (!model_.initString(urdf_string_))
{
  ...
  RCLCPP_ERROR_STREAM(
    get_node()->get_logger(),
    "Joint '" << joint_name_ << "' was not found in robot_description"
  );
  return controller_interface::CallbackReturn::ERROR;
}
...
if (!joint_->limits) {
  RCLCPP_ERROR_STREAM(
    get_node()->get_logger(),
    "Joint '" << joint_name_ << "' has no URDF limits"
  );
  return controller_interface::CallbackReturn::ERROR;
}
...
joint_names.push_back(joint_name_ + "/" + hardware_interface::HW_IF_VELOCITY);
joint_names.push_back(joint_name_ + "/" + hardware_interface::HW_IF_EFFORT);
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  calcTorque =
  servoCmd.posStiffness * (servoCmd.pos - currentPos)
  ...
  if (!std::isfinite(calcTorque))
{
  RCLCPP_WARN_THROTTLE(
    ...
    *get_node()->get_clock(),
    1000,
    "NaN/Inf torque detected on joint %s. Setting torque to 0.",
    joint_name_.c_str()
  );
...
assert (state_interfaces_[1].get_interface_name() == hardware_interface::HW_IF_VELOCITY);
assert (state_interfaces_[2].get_interface_name() == hardware_interface::HW_IF_EFFORT);

  command_interfaces_[0].set_value(calcTorque); // Send effort to joint

  lastState.q = (float) currentPos;
  lastState.dq = (float) currentVel;
  lastState.tau_est = (float) state_interfaces_[2].get_value(); // May cause issues? -> Possibly causing issues. Also check (float) conversion issues
