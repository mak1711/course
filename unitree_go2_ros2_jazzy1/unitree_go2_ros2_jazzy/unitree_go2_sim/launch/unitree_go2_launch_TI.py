...
def generate_launch_description():
    ...
    # See unitree_go2_launch.py for why this is needed: it isolates this launch's
    # gz-transport bus so a stale "gz sim" process left over from a previous run
    # can't cause this run's robot spawn / controllers to silently fail.
    set_gz_partition = SetEnvironmentVariable(
        "GZ_PARTITION", f"unitree_go2_sim_ti_{os.getpid()}_{int(time.time())}"
    )

    # See unitree_go2_launch.py for why this is needed: ros-humble-gz-ros2-control
    # ships no environment hook to put its plugin on gz-sim's search path, so
    # without this, controller_manager never starts and every controller spawner
    # in this launch hangs forever with no error printed anywhere.
