...
void setSimStanceGain(int legID){
    // These were previously set to the *real-robot* gains (60/5, 60/5, 80/7) --
    // upstream unitree_guide uses much stiffer gains for simulation (180/8,
    // 180/8, 300/15) because sim has no motor-side current loop underneath the
    // position PD. The 250Hz control rate this was tested at was too coarse to
    // hold these gains stably (contact-induced ringing); that's fixed by raising
    // controller_manager's update_rate (see unitree_go2_sim/config/ros_control/
    // ros_control.yaml) to match the physics step instead of lowering Kp/Kd.
