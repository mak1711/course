...
Go2Robot::Go2Robot(){
    ...
    _feetPosNormalStand <<  0.1934,  0.1934, -0.1934, -0.1934,
    ...
    // Go2 total mass (trunk + 4 legs, from unitree_go2_description URDF link masses) is
    // ~15.1kg, versus Go1's 12kg -- reusing Go1Robot's numbers here previously starved
    // the balance QP's gravity/inertia compensation by ~20%, which is why Trotting
    // immediately toppled the robot. Inertia has no full system ID for this model, so
    // it's Go1's diagonal estimate scaled by the mass ratio (15/12) as a starting point.
#ifdef COMPILE_WITH_REAL_ROBOT
    _mass = 15.0;
    _pcb << 0.0, 0.0, 0.0;
    _Ib = Vec3(0.099, 0.2606, 0.2831).asDiagonal();
#endif  // COMPILE_WITH_REAL_ROBOT
